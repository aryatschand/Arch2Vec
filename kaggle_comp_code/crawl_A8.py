...
filtered_data = data[(data['QCode'] == 'A8') & (data['Status'] == 'accepted') & (data['Language'].str.contains('C\+\+', na=False))]

runtimes = []
renamed_good_programs = []

# Create a directory for storing the source files and executables
os.makedirs('code_files/A8', exist_ok=True)
...
for index, row in filtered_data.iterrows():
    ...
    if not solution_code_row.empty:
        ...
        code_file = f"code_files/A8/solution_{index}.cpp"
        with open(code_file, 'w') as f:
            f.write(code)

        # Compile the code using g++ with -O0
        executable = f"code_files/A8/solution_{index}.out"
...
plt.title('Histogram of Runtimes for A8 Accepted Solutions')
