    ...
    //          assert(_nodecnt==0 || _nodecnt<nodes.size());
        ...
        void update ( int no, int L, int R, int l, int r,const T& v ) {
            assert ( R>L );
            assert ( no<_nodecnt );
            node &current=nodes[no];
            T& currentV=current.val;
            if ( l<=L && R<=r ) {
                currentV.update ( v );
                return;
            }
            ...
            currentV.updateChildren ( nodes[current.l].val,nodes[current.r].val );
    
    
    
            int s= ( L+R ) >>1;
            if ( l<s ) {
                update ( current.l,L,s,l,r,v );
            }
            if ( r>s ) {
                update ( current.r,s,R,l,r,v );
            }
            currentV.merge ( nodes[current.l].val,nodes[current.r].val );
    
        }
        void update ( int l, int r,const T&v ) {
            if ( r<=l ) return;
            return update ( 0,Imin,Imax,l,r,v );
        }
        T query ( int no, int L, int R, int l, int r ) {
            ...
            T res;
            if ( l<s ) {
                T res2=query ( current.l,L,s,l,r );
                res= res.combine ( res2 );
            }
            if ( r>s  &&current.r ) {
    
                T res2= query ( current.r,s,R,l,r );
                res=res.combine ( res2 );
            }
            return res;
        }
